...
int main() {
 ...
 // font.height=5;
  ...
  // {
  //   char *temp;
  //   temp[0]=(char)image_buf[i];
  //   ST7735_WriteString(0,0,temp,font,0,0);
  // }
   uint8_t image_buf_2[324*324];
   index=0;
   for (int y = 0; y < 160; y++) {
	    for (int x = 0; x < 80; x++) {
              image_buf_2[index++] = image_buf[(2+320-2*y)*324+(2+40+2*x)];
      }
   }

   for (int x=0; x<160*80;x++)
   {
    if(image_buf_2[x]>128)image_buf_2[x]=7;
    else image_buf_2[x]=0;
   }
	...
	while(1)
	{
		for (i=0; i<640; i++) {
            ...
            for (j=0; j<480; j++) {
               ...
               if(j>=80) drawPixel(i, j, BLACK) ;
               else if(i>=160) drawPixel(i, j, BLACK) ;
               else if(i<160&&j<80) drawPixel(i, j, image_buf_2[i*160+j]) ;
